...
size_t g_profilerValueIndex;

Profiler g_gpuFrameProfiler;
Profiler g_presentProfiler;
Profiler g_updateDirectorProfiler;
Profiler g_renderDirectorProfiler;
Profiler g_frameFenceProfiler;
Profiler g_presentWaitProfiler;
Profiler g_swapChainAcquireProfiler;

bool g_profilerVisible;
bool g_profilerWasToggled;

static constexpr bool g_vulkan = true;

static bool g_triangleStripWorkaround = false;

bool g_hardwareResolve = true;
bool g_hardwareDepthResolve = true;

std::unique_ptr<RenderInterface> g_interface;
std::unique_ptr<RenderDevice> g_device;

RenderDeviceCapabilities g_capabilities;
...
std::unique_ptr<RenderCommandList> g_commandLists[NUM_FRAMES];
...
std::unique_ptr<RenderCommandFence> g_copyCommandFence;

std::unique_ptr<RenderSwapChain> g_swapChain;
bool g_swapChainValid;
...
std::unique_ptr<RenderTexture> g_blankTextures[TEXTURE_DESCRIPTOR_NULL_COUNT];
static std::unique_ptr<RenderTextureView> g_blankTextureViews[TEXTURE_DESCRIPTOR_NULL_COUNT];

TextureDescriptorAllocator g_textureDescriptorAllocator;
...
std::atomic<uint32_t> g_pipelinesCreatedInRenderThread;
std::atomic<uint32_t> g_pipelinesCreatedAsynchronously;
std::atomic<uint32_t> g_pipelinesDropped;
std::atomic<uint32_t> g_pipelinesCurrentlyCompiling;
std::string g_pipelineDebugText;
Mutex g_debugMutex;
#endif
...
std::atomic<uint32_t> g_compilingPipelineTaskCount;
std::atomic<uint32_t> g_pendingPipelineTaskCount;
...
UploadAllocator g_uploadAllocators[NUM_FRAMES];
...
moodycamel::BlockingConcurrentQueue<RenderCommand> g_renderQueue;
